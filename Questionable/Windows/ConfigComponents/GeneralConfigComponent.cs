...
internal sealed class GeneralConfigComponent : ConfigComponent
{
    ...
    public override void DrawTab()
    {
        ...
        {
            ImGui.Separator();
            ImGui.Text(_L("DailyRoutines compatibility"));
            using (ImRaii.PushIndent())
            {
                bool configureDailyRoutines = Configuration.General.ConfigureDailyRoutines;
                if (ImGui.Checkbox(_L("Temporarily disable conflicting DailyRoutines modules while questing"),
                ...
                ImGuiComponents.HelpMarker($"{string.Join("\n", _dailyRoutinesIpc.ConflictingModules)}\n" +
                                           _L("If another module conflicts with questing, report it to the CN maintainer."));

                bool useDailyRoutinesTeleport = Configuration.General.UsingDailyRoutinesTeleport;
                if (ImGui.Checkbox(_L("Use DailyRoutines for aethernet teleporting"),
                ...
                ImGuiComponents.HelpMarker(_L("Uses DailyRoutines BetterTeleport. If the module is not enabled, Questionable will try to enable it temporarily. Disable this option if teleporting behaves incorrectly."));
            }
        }
    }
}
